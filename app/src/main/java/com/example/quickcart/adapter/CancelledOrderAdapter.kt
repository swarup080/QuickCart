...
class CancelledOrderAdapter(private val orderList: ArrayList<Order>, private val context: Context) :
    RecyclerView.Adapter<CancelledOrderAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(private val binding: StatusUiBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(order: Order) {
            binding.apply {
                ...
                val cartItemAdapter = CartItemAdapterForCancelled(order.cartItems, order.deliveryDate)
...
class CartItemAdapterForCancelled(private val cartItems: List<CartItem>, private val deliveryDate: Long) :
    RecyclerView.Adapter<CartItemAdapterForCancelled.CartItemViewHolder>() {
    ...
    inner class CartItemViewHolder(private val binding: StatusUiChildBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(cartItem: CartItem) {
            binding.apply {
                ...
                // Set delivery status
                productDeliverStatus.setTextColor(Color.parseColor("#fd3434"))
                if (deliveryDate > 0) {
                    ...
                    productDeliverStatus.text = "Cancelled on, $deliveryDateFormatted"
                } else {
